...
$nama = $conn->real_escape_string($data['nama_dosen']);
...
if ($conn->query("INSERT INTO users (email,username, password,role) VALUES ('$email', '$username', '$password','dosen')")) {
    $id_user = $conn->insert_id;

    // Insert ke tabel dosen
    $query = "INSERT INTO dosen (nama_dosen, no_telepon, nip, id_user) 
          VALUES ('$nama', '$telepon', '$nip', $id_user)";
    if ($conn->query($query)) {
        echo json_encode(["success" => true]);
    } else {
        echo json_encode(["success" => false, "error" => "Gagal insert dosen: " . $conn->error]);
    }
} else {

...
$nama = $conn->real_escape_string($data['nama_asdos']);
...
if ($conn->query("INSERT INTO users (email,username, password,role) VALUES ('$email', '$username', '$password','asdos')")) {
    $id_user = $conn->insert_id;

    // Insert ke tabel asdos
    $query = "INSERT INTO asdos (nama_asdos, no_telepon, nip, id_user) 
          VALUES ('$nama', '$telepon', '$nip', $id_user)";
    if ($conn->query($query)) {
        echo json_encode(["success" => true]);
    } else {
        echo json_encode(["success" => false, "error" => "Gagal insert asdos: " . $conn->error]);
    }
} else {
